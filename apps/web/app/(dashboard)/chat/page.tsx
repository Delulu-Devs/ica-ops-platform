...
// Mock component for Socket.io integration
function useChatSocket(roomId: string | null, onMessage: () => void) {
  useEffect(() => {
    if (!roomId) return;
...
export default function ChatPage() {
  ...
  useChatSocket(selectedRoom, () => refetchMessages());
        ...
        <ScrollArea className="flex-1">
          <div className="flex flex-col gap-1 p-2">
            {isRoomsLoading ? (
               ...
               <div className="text-center text-muted-foreground p-4 text-sm">No conversations yet.</div>
            ) : (
              rooms?.map((room) => (
                <button
                  key={room.roomId}
                ...
                >
                  <div className="font-medium truncate w-full">
                    {room.roomId.startsWith('dm:') ? 'Direct Message' : 'Batch Chat'}
                  </div>
          ...
          <>
            ...
            <CardFooter className="p-4 border-t">
              <form onSubmit={handleSendMessage} className="flex w-full items-center gap-2">
                ...
                />
                <Button size="icon" type="submit" disabled={sendMessageMutation.isPending || !messageInput.trim()}>
