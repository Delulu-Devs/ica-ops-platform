...
import { z } from 'zod';
import { coaches, demos, students, subscriptions } from '../db/schema';
...
function _getCount(result: Array<{ count: number }> | undefined, _key: 'count' = 'count'): number {
  return result?.[0]?.count ?? 0;
}

export const analyticsRouter = router({
  // Get conversion funnel metrics
  getFunnel: adminProcedure.input(dateRangeSchema).query(async ({ ctx, input }) => {
    const conditions = [];
    if (input.startDate) conditions.push(gte(demos.createdAt, new Date(input.startDate)));
    if (input.endDate) conditions.push(lte(demos.createdAt, new Date(input.endDate)));
    ...
    return {
      ...
      interestRate: attended > 0 ? Math.round((interested / attended) * 100) : 0,
      conversionRate: interested > 0 ? Math.round((converted / interested) * 100) : 0,
      statusBreakdown: counts,
    };
  }),

  // Get coach performance metrics
  getCoachPerformance: adminProcedure.input(dateRangeSchema).query(async ({ ctx, input }) => {
    const coachList = await ctx.db.select({ id: coaches.id, name: coaches.name }).from(coaches);
    ...
    const performance = await Promise.all(
      coachList.map(async (coach: { id: string; name: string }) => {
        ...
        return {
          ...
          noShows,
          studentCount,
          attendanceRate: totalDemos > 0 ? Math.round((attended / totalDemos) * 100) : 0,
  ...
  getDashboard: adminProcedure.query(async ({ ctx }) => {
    ...
    const todayDemosResult = await ctx.db
      .select({ count: sql<number>`count(*)::int` })
      .from(demos)
      .where(and(gte(demos.scheduledStart, startOfDay), lte(demos.scheduledStart, endOfDay)));
